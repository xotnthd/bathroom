...
function fn_tempFailUpdateForm(){
	var f = document.form1;
	f.action = "<c:url value='/usr/member/suggestionTempFailUpdate.do' />";
	f.submit();
}
...
<div>
	...
	<div id="inputVal1">
		...
		<div>
			...
			<c:choose>
	            <c:when test="${fn:length(failList) > 0}">
	                <c:forEach items="${failList }" var="list" varStatus="i">
	                	<c:if test="${list.RETURN_POSITION eq 1 }">
	                		<p style="color: red;">${list.RETURN_COMMENT }</p>
	                	</c:if>
	                </c:forEach>
                </c:when>
             </c:choose>
		...
		<div>
			...
			* 전화번호(필수) <input type="text" name="p_phone_no" id="p_phone_no" value="${map.PHONE_NO }"/></br>
			  ...
			  <c:choose>
	            <c:when test="${fn:length(failList) > 0}">
	                <c:forEach items="${failList }" var="list" varStatus="i">
	                	<c:if test="${list.RETURN_POSITION eq 2 }">
	                		<p style="color: red;">${list.RETURN_COMMENT }</p>
	                	</c:if>
	                </c:forEach>
                </c:when>
		...
		<div>
			...
			* 영문주소(필수) 	<input type="text" name="p_addr_eng" id="p_addr_eng" value="${map.ADDR_ENG }"/><input type="text" name="p_addr_eng2" id="p_addr_eng2" value="${map.ADDR_ENG2 }"/></br>
			  ...
			  <c:choose>
	            <c:when test="${fn:length(failList) > 0}">
	                <c:forEach items="${failList }" var="list" varStatus="i">
	                	<c:if test="${list.RETURN_POSITION eq 3 }">
	                		<p style="color: red;">${list.RETURN_COMMENT }</p>
	                	</c:if>
	                </c:forEach>
                </c:when>
	...
	<div id="inputVal2" style="display:none;">
		...
		<div>
			...
			* 영문명(필수) <input type="text" name="p_product_nm_eng" id="p_product_nm_eng" value="${map.PRODUCT_NM_ENG }"/></br>
			  ...
			  <c:choose>
	            <c:when test="${fn:length(failList) > 0}">
	                <c:forEach items="${failList }" var="list" varStatus="i">
	                	<c:if test="${list.RETURN_POSITION eq 4 }">
	                		<p style="color: red;">${list.RETURN_COMMENT }</p>
	                	</c:if>
	                </c:forEach>
                </c:when>
		...
		<c:choose>
            <c:when test="${fn:length(fileList) > 0}">
                <c:forEach items="${fileList }" var="list" varStatus="i">
                	...
                	<div>
                		전성분표 &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type="file" id="file0" name="file_0" /></br>
                	...
                	<div>
                		패키지  &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input type="file" name="file1" id="file_1" /> </br> 
		...
		<div>
			...
			<c:choose>
	            <c:when test="${fn:length(failList) > 0}">
	                <c:forEach items="${failList }" var="list" varStatus="i">
	                	<c:if test="${list.RETURN_POSITION eq 5 }">
	                		<p style="color: red;">${list.RETURN_COMMENT }</p>
	                	</c:if>
	                </c:forEach>
                </c:when>
		...
		<div style="display:inline-block;">
			...
			<button value="약관동의 후 카테고리 선택하기" onclick="fn_tempFailUpdateForm();">재 승인요청</button>
		</div>
		</br>
		</br>
		</br>
