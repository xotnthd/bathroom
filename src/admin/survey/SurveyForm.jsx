...
const SurveyForm = () => {
    ...
    const fetchSurveyDetail = async () => {
        ...
        if (res.ok) {
            ...
            // 계층삭제            const questions = (data.questions || []).map(q => {
        ...
        <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
            ...
            <div style={{ background: '#fff', padding: '20px', borderRadius: '8px', border: '1px solid #ddd', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '15px', alignItems: 'center' }}>
                    <label style={{ fontWeight: 'bold' }}>등�플�삭제��등</label>
                    <div>
                        <label style={{ marginRight: '15px' }}><input type="radio" checked={form.templateYn === 'Y'} onChange={() => setForm({...form, templateYn: 'Y'})} /> 등�플�?(초안)</label>
                        <label><input type="radio" checked={form.templateYn === 'N'} onChange={() => setForm({...form, templateYn: 'N'})} /> 등�반 등�문 (배포삭제</label>
                    </div>

                    <label style={{ fontWeight: 'bold' }}>등�문 등�목</label>
                    <input type="text" value={form.survNm} onChange={e => setForm({...form, survNm: e.target.value})} style={{ padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }} placeholder="텍스트" value={form.survExpl || ''} onChange={e => setForm({...form, survExpl: e.target.value})} style={{ padding: '8px', border: '1px solid #ccc', borderRadius: '4px', height: '60px', resize: 'vertical' }} placeholder="텍스트" value="Y">등�용</option>
