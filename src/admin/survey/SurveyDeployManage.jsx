...
const SurveyDeployManage = () => {
    ...
    const fetchSurveyList = async () => {
        const res = await apiClient('/admin/api/survey/list', {
            ...
            body: JSON.stringify({ page, limit, templateYn: 'N' })
        });
        if (res.ok) {
    ...
    return (
        <div style={{ padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <h2 style={{ margin: 0 }}>배포 설문 관리</h2>
                {rgstYn === 'Y' && (
                    <button onClick={() => navigate('/admin/survey/deploy/write')} style={{ padding: '10px 20px', background: '#3498db', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
                        + 신규 배포 설문 등록
                    </button>
                )}
            </div>
                    ...
                    {surveyList.map(s => (
                        <tr key={s.survId} style={{ borderBottom: '1px solid #eee' }}>
                            <td style={{ padding: '12px' }}>
                                <span style={{ background: '#2ecc71', color: '#fff', padding: '3px 6px', borderRadius: '4px', fontSize: '12px' }}>배포설문</span>
                            </td>
                            <td style={{ padding: '12px' }}>{s.survId}</td>
                            <td style={{ padding: '12px', color: '#2980b9', cursor: 'pointer' }} onClick={() => navigate(`/admin/survey/deploy/write?survId=${s.survId}`)}>
                                {s.survNm}
                            </td>
