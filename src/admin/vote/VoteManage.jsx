...
const VoteManage = () => {
    ...
    const handleSearch = () => fetchVoteList(searchKeyword);
    const handleReset = () => { setSearchKeyword(''); fetchVoteList(''); };

    const handleDelete = async (e, voteIdx) => {
        e.stopPropagation();
        if (delYn !== 'Y') { alert('삭제 권한이 없습니다.'); return; }
        const ok = await deleteVote(voteIdx);
        if (ok) fetchVoteList(searchKeyword);
    };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '20px' }}>
            ...
            <div className="admin-card">
                <div className="admin-card-header" style={{ padding: '12px 20px', flexWrap: 'wrap', justifyContent: 'flex-start', gap: '10px 24px' }}>
                    <span className="admin-card-title">투표 관리</span>
            ...
            <div className="admin-card" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div className="admin-card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    {rgstYn === 'Y' && (
                        <button onClick={() => navigate('/admin/vote/write')} className="admin-btn admin-btn-primary" style={{ padding: '6px 16px', flexShrink: 0, whiteSpace: 'nowrap' }}>+ 투표 등록</button>
                    )}
                </div>
                <div className="admin-card-body" style={{ flex: 1, overflowY: 'auto', padding: '0' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px', textAlign: 'center' }}>
                        ...
                        <tbody>
                            {voteList.map(v => (
                                <tr key={v.idx} onClick={() => navigate(`/admin/vote/write?voteIdx=${v.idx}`)} style={{ cursor: 'pointer', borderBottom: '1px solid var(--border-color, #eee)' }} className="admin-table-row-hover">
                                    ...
                                    {delYn === 'Y' && (
                                        <td style={{ padding: '12px' }}>
                                            <button onClick={(e) => handleDelete(e, v.idx)} className="admin-btn admin-btn-danger" style={{ padding: '4px 10px', fontSize: '12px' }}>삭제</button>
                                        </td>
                                    )}
                                </tr>
                            ))}
                            {voteList.length === 0 && (
                                <tr><td colSpan={delYn === 'Y' ? 6 : 5} style={{ padding: '40px', textAlign: 'center', color: '#95a5a6' }}>등록된 투표가 없습니다.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
